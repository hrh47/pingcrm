...
const FlashMessages = ({ className }) => {
  const success = useFlashMessage((state) => state.success);
  const error = useFlashMessage((state) => state.error);
  const clear = useFlashMessage((state) => state.clear);
      ...
      {success && (
        <div className="flex items-center justify-between mb-8 max-w-3xl bg-green-500 rounded">
          <div className="flex items-center">
            ...
            <div className="py-4 text-white text-sm font-medium">{success}</div>
          </div>
          <button
            type="button"
      ...
      {error && (
        <div className="flex items-center justify-between mb-8 max-w-3xl bg-red-500 rounded">
          <div className="flex items-center">
            ...
            <div className="py-4 text-white text-sm font-medium">{error}</div>
          </div>
          <button
            type="button"
            className="group mr-2 p-2"
